<?php
    ...
    //fetch courses
    $courses = mysqli_query($con,"SELECT * FROM `course_comm` ORDER BY comm_course_id ASC");
...
$comm = $_SESSION['views'];
  ...
  $sql = "INSERT INTO `course_view`(`sci`) VALUES ($comm)";
...
<div class="bg1" id="about_us">
	<div class="container">
		<div class="col-sm-6">
			<p class="index_about_company">Commerce</p>
			<p class="index_about_para">EduWorld provides the stream of Chartered Accountancy(CA) and Cost and Management Accountancy(CMA).<br><br> Nowadays it is very important to aware children from all the economic and financial situations taking place in the world and make them understand the importance of it.<br><br> 
                Thus, we provide those excellent standards of quality education at affordable fees.</p>
		</div>
		
		<div class="col-sm-6"><img src="images/about_com.jpg" class="img-responsive" style="padding:20px 0;"></div>
	...
	<div class="container">
       ...
       ?>
			<div class="col-sm-3 home_services_top_padding">
				<p><i class="fa fa-book home_testimonial_quote" aria-hidden="true"><?php echo $row['comm_course_name'];?></i></p>
				<p class="index_testimoinial_para"><?php echo $row['comm_course_desc'] ?></p>
				<p class="index_testimoinial_para">New Batch Starting From: &nbsp <?php echo $row['comm_course_batch'] ?></p>
				<p><img class="index_testimonial_img" src="<?php echo $row['comm_faculty_img_src']; ?>"><span class="index_testimonial_name"><span class="index_testimonial_blue_name"><?php echo $row['comm_course_faculty'];?></p>
			</div>
            <?php } ?>
</div>
    </div></div>
